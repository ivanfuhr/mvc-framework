...
class RouterCore
{
    ...
    protected static function analyzeRoute($route)
    {
        ...
        //ROUTE ================================================
        $regrex = '/\[(.*?)\]/';                                             //Regrex para encontrar os valores que estão entre []
        ...
        preg_match_all($regrex, $route, $results);                           //Função que aplica o Regrex na variavel $route e devolve $results com o resultado 
        ...
        foreach ($explodeRoute as $keyRoute => $route) {                         //Criado um laço que percorre todas as posições do array referente a rota
            ...
            foreach ($results[0] as $keyResult => $result) {                         //Laço que percorre as posições referente a argumentos que ficam dentro de []
                if ($result === $route) {                                            //Se a posição da rota for a mesma que a posição do resultado ele entenderá está posição como um argumento
                    $args[$results[1][$keyResult]] = $countURI[$keyRoute] ?? null;   //Então ele adiciona a um array chamado $args está possição que deve ser a mesma da URI
